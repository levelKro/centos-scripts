...
while true; do
    read -p "Avez-vous besoin d'installer le Repository Forge (ATTENTION: Ne pas installer si déjà présente) ?" ynrpm
    case $ynrpm in
        [YyOo]* ) 
...
echo "### Actualisation du systeme et retrait des librairies possiblement conflictuels"
yum -y -q update
yum -y -q remove libvpx libogg libvorbis libtheora libx264 x264 ffmpeg nasm

echo "### Installation des pre-requis"
yum -y -q install glibc gcc gcc-c++ autoconf automake libtool git make pkgconfig subversion
...
#yum -y -q install libogg libvorbis vorbis-tools lame lame-devel libtheora theora-tools nasm
...
rpm -iv --quiet https://www.nasm.us/pub/nasm/stable/linux/nasm-2.14.02-0.fc27.x86_64.rpm
...
while true; do
    ...
    case $ynxvid in
        [YyOo]* ) 
			echo "### Librairie XVID"
			cd /opt
			wget -q http://downloads.xvid.org/downloads/xvidcore-1.3.2.tar.gz
			tar xzf xvidcore-1.3.2.tar.gz
			rm -f xvidcore-1.3.2.tar.gz
			cd xvidcore/build/generic
			./configure --quiet --prefix="$HOME/ffmpeg_build"
			make -s
			make -s install	
			make -s clean 
			read -p "### Pas d'erreur ? Appuyer sur [ENTRER] pour continuer... (ou CTRL+C pour annuler)"
...
while true; do
    ...
    case $ynogg in
        [YyOo]* ) 
			echo "### Librairie OGG"
			cd /opt
			wget -q http://downloads.xiph.org/releases/ogg/libogg-1.3.3.tar.gz
...
while true; do
    ...
    case $yntheora in
        [YyOo]* ) 
			...
			cd libtheora-1.1.1
			./configure --quiet --prefix="$HOME/ffmpeg_build" --with-ogg="$HOME/ffmpeg_build" --disable-examples --disable-shared --disable-sdltest --disable-vorbistest
			make -s
			make -s install
			make -s clean
			read -p "### Pas d'erreur ? Appuyer sur [ENTRER] pour continuer... (ou CTRL+C pour annuler)"
...
while true; do
    ...
    case $ynaac in
        [YyOo]* ) 
			echo "### Librairie AAC"
			cd /opt
			wget -q http://downloads.sourceforge.net/opencore-amr/vo-aacenc-0.1.2.tar.gz
...
while true; do
    ...
    case $ynfaac in
        [YyOo]* ) 
			echo "### Librairie FAAC"
			cd /opt
			wget -q http://downloads.sourceforge.net/faac/faac-1.28.tar.gz
...
while true; do
    ...
    case $ynvpx in
        [YyOo]* ) 
			...
			./configure --quiet --prefix="$HOME/ffmpeg_build" --disable-examples
			make -s
			make -s install
			make -s clean
			read -p "### Pas d'erreur ? Appuyer sur [ENTRER] pour continuer... (ou CTRL+C pour annuler)"
...
while true; do
    ...
    case $ynx264 in
        [YyOo]* ) 
			...
			./configure --quiet --prefix="$HOME/ffmpeg_build" --bindir="$HOME/bin" --enable-static --disable-asm
			make -s
			make -s install
			make -s clean
			read -p "### Pas d'erreur ? Appuyer sur [ENTRER] pour continuer... (ou CTRL+C pour annuler)"
...
#OPUS 1.2.1
while true; do
    read -p "### Besoin de compiler codec OPUS ?" ynopus
    case $ynopus in
        [YyOo]* ) 
			echo "### Librairie Opus"
			cd /opt
			wget -q https://archive.mozilla.org/pub/opus/opus-1.2.1.tar.gz
...
#FDK-AAC LATEST
while true; do
    read -p "### Besoin de compiler codec FDK-AAC ?" ynfdkaac
    case $ynfdkaac in
        [YyOo]* ) 
			echo "### Librairie FDK-AAC"
			cd /opt
...
#FFMPEG 2.5
echo "### Compilation du FFMpeg avec les librairies créés"
...
make -s install
make -s clean
echo "### Installation complete, faites 'ffmpeg -v' pour vérifier"
